...
def ensure_3d(data):
    ...
    if data.ndim == 3:
        return data
    raise ValueError(f"Expected EEG data with shape (N, T) or (N, T, C), got {data.shape}")


def resample_data(data, orig_points, target_points):
    """
    将数据从 orig_points 个采样点重采样到 target_points 个采样点
    data shape: (N, orig_points) or (N, orig_points, C)
    返回 shape: (N, target_points) or (N, target_points, C)
    """
    data = ensure_3d(data)
    resampled = resample(data, target_points, axis=1)
    return resampled


def get_rms(records):
    records = np.asarray(records, dtype=np.float32)
    return math.sqrt(float(np.mean(records ** 2)))
...
def prepare_data(EEG_all, noise_all, combin_num, train_per, noise_type):
    ...
    EEG_all_random = random_signal(signal=EEG_all, combin_num=1)[0]
    noise_all_random = random_signal(signal=noise_all, combin_num=1)[0]
    ...
    channel_num = noise_all_random.shape[2]
    ...
    EEG_train = random_signal(signal=train_eeg, combin_num=combin_num).reshape(
        combin_num * train_eeg.shape[0], timepoint, channel_num
    )
    NOISE_train = random_signal(signal=train_noise, combin_num=combin_num).reshape(
        combin_num * train_noise.shape[0], timepoint, channel_num
    )
    ...
    for i in range(noiseEEG_train.shape[0]):
        ...
        std_train = np.std(noiseEEG_train[i])
        eeg_train_all_std = EEG_train[i] / std_train
        EEG_train_end_standard.append(eeg_train_all_std)

        noiseeeg_train_end_standard = noiseEEG_train[i] / std_train
